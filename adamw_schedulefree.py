...
class AdamWScheduleFree(Optimizer):
    ...
    Args:
        ...
        weight_decay: Decoupled weight decay (L2) applied to y-parameters. Defaults to 0.0.
        ...
        weight_lr_power: Power of lr-max used during warmup weighting. Defaults to 2.0.
        state_dtype: Optional dtype for state variables, e.g., "float32". Defaults to None.
    ...
    def __init__(
        ...
        **kwargs: object,  # type: ignore[ANN003]
        ...
        if not (beta_1 > 0.0):
            raise ValueError(
                "beta_1 must be strictly positive for Schedule-Free evaluation."
            )
        super().__init__(
    ...
    def build(self, var_list: list[object]) -> None:
        ...
        if self.built:
            return
        if not var_list:
            raise ValueError("var_list must be non-empty to build optimizer state.")
        super().build(var_list)

        # Create per-variable state using the reference dtype/shape (dtype override not supported here).
    ...
    def _prepare_step(self, trainable_variables: list[object]) -> None:
        ...
        Computes:
            - Linear warmup schedule s_k in (0, 1].
            - Running lr_max for warmup weighting.
            - Weighted average coefficient c_{k+1}.
            - Initializes z = y at step 0.
        ...
        if self.warmup_steps > 0:
            warm = ops.minimum(
                k / ops.cast(self.warmup_steps, dtype), ops.cast(1.0, dtype)
            )
            sched = warm
        else:
        ...
        weight = ops.power(k, ops.cast(self.r, dtype)) * ops.power(
            self._lr_max, ops.cast(self.weight_lr_power, dtype)
        )
    ...
    def _backend_apply_gradients(self, grads, trainable_variables) -> None:  # type: ignore[override]
        """Hook to prepare step-wise scalars once per optimizer step.
    ...
    def update_step(
        ...
        # y += c (z - y) + gamma * (beta1 * (1 - c) - 1) * grad_norm
    ...
    def get_config(self) -> dict:
        ...
        Returns:
            dict: A dictionary containing the optimizer configuration.
        """
    ...
    def get_eval_weights(self, model: object) -> list[object]:
        ...
        x-weights are defined as x = (y - (1 - beta1) * z) / beta1.

        Args:
            model: A built Keras model whose trainable weights match optimizer state.

        Returns:
            list[object]: Tensors for x-weights aligned with `model.trainable_weights`.
        ...
        if self.beta_1 <= 0.0:
            raise ValueError(
                "beta_1 must be > 0 to compute evaluation weights x from (y, z)."
            )
...
class ScheduleFreeEvalCallback(callbacks.Callback):
    ...
    def on_test_begin(self, logs: dict | None = None) -> None:
        ...
        Args:
            logs: Optional logs dictionary (unused).
        """
    ...
    def _compute_x_from_state(self, base_opt: object) -> list[object]:
        ...
        if beta_1 is None or z_vars is None:
            raise RuntimeError(
                "ScheduleFreeEvalCallback needs either optimizer.get_eval_weights(model) "
    ...
    def _restore_backup_weights(self) -> None:
        ...
        if self.model is None or self._backup_weights is None:
            return
        for var, w_np in zip(
            self.model.trainable_weights, self._backup_weights, strict=True
        ):
